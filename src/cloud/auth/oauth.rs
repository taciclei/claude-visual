...
pub(crate) async fn fetch_user_profile(
    ...
    let (url, email_url) = match provider {
        ...
        OAuthProvider::Email => {
            return Err(AuthError::OAuth(
                "Email provider not supported for profile fetch".to_string(),
            ))
        }
    };
    ...
    match provider {
                ...
                if let Some(email_url) = email_url {
                        ...
                        .await
                    {
                        if let Ok(emails) = email_response.json::<Vec<GitHubEmail>>().await {
                            email = emails.into_iter().find(|e| e.primary).map(|e| e.email);
                        }
                    }
                }
            }
        ...
        OAuthProvider::Google => Ok(UserProfile {
            id: profile_data["id"].as_str().unwrap_or_default().to_string(),
