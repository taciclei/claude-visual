...
pub(crate) fn convert_variant(family_name: &str, variant: &ZedThemeVariant) -> Result<Theme> {
    ...
    let background = parse_color(style.background.as_deref())
        ...
        .unwrap_or_else(|| if is_dark { default_dark_bg() } else { default_light_bg() });

    let surface = parse_color(style.element_background.as_deref())
        .unwrap_or_else(|| if is_dark { default_dark_surface() } else { default_light_surface() });

    let surface_hover = parse_color(style.element_hover.as_deref())
        .unwrap_or_else(|| if is_dark { default_dark_surface_hover() } else { default_light_surface_hover() });

    let border = parse_color(style.border.as_deref())
        .unwrap_or_else(|| if is_dark { default_dark_border() } else { default_light_border() });

    let text = parse_color(style.text.as_deref())
        .unwrap_or_else(|| if is_dark { default_dark_text() } else { default_light_text() });

    let text_muted = parse_color(style.text_muted.as_deref())
        .or_else(|| parse_color(style.text_placeholder.as_deref()))
        .unwrap_or_else(|| if is_dark { default_dark_text_muted() } else { default_light_text_muted() });

    // Get accent color from players or text.accent
    let accent = parse_color(style.text_accent.as_deref())
        .or_else(|| {
            style.players.as_ref()
                .and_then(|p| p.first())
    ...
    let focus_ring = parse_color(style.border_focused.as_deref())
        .unwrap_or_else(|| {
            if is_dark {
    ...
    let selection = style.players.as_ref()
        .and_then(|p| p.first())
    ...
    let colors = ThemeColors {
        ...
        success: parse_color(style.status_success.as_deref())
            .unwrap_or_else(default_success),
        warning: parse_color(style.status_warning.as_deref())
            .unwrap_or_else(default_warning),
        error: parse_color(style.status_error.as_deref())
            .unwrap_or_else(default_error),
        info: parse_color(style.status_info.as_deref())
...
pub(crate) fn convert_syntax(syntax: Option<&HashMap<String, SyntaxStyle>>, is_dark: bool) -> SyntaxColors {
    ...
    SyntaxColors {
        ...
        punctuation: get_color(&["punctuation", "punctuation.delimiter", "punctuation.bracket"]),
    }
}
