...
impl ChatView {
    ...
    /// Alias for cycle_message_filter
    pub fn next_filter(&mut self, cx: &mut Context<Self>) {
        self.cycle_message_filter(cx);
    }

    /// Get message filter (alias for message_filter)
    pub fn get_message_filter(&self) -> MessageFilter {
        self.message_filter
    }

    /// Check if a message is visible with current filters
    pub fn is_message_visible(&self, index: usize, cx: &Context<Self>) -> bool {
        if let Some(msg) = self.messages.get(index) {
            ...
            if self.show_bookmarked_only {
                if let Some(view) = self.message_views.get(index) {
                    return view.read(cx).is_bookmarked();
                }
                return false;
            }
            true
    ...
    /// Get count of filtered messages (alias for visible_message_count)
    pub fn filtered_message_count(&self, cx: &Context<Self>) -> usize {
        self.visible_message_count(cx)
    }
