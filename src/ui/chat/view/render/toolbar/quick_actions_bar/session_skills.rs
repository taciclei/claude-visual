...
impl ChatView {
    pub(super) fn render_session_skills(
        &self,
        theme: &crate::app::theme::Theme,
        cx: &mut Context<Self>,
    ) -> impl IntoElement {
        // Get popular skills from session (if any)
        let session_skills: Vec<String> = self
            .session_info
            .as_ref()
            .map(|info| {
                info.skills
        ...
        div()
            ...
            .when(show_defaults, |d| {
                ...
                .child(div().w(px(1.0)).h(px(12.0)).bg(theme.colors.border).mx_1())
            })
            ...
            .when(!show_defaults, |d| {
                d.children(session_skills.iter().enumerate().map(|(idx, skill)| {
                    ...
                    let skill_icon = if skill.contains("commit") {
                        ...
                        "🐛"
                    } else if skill.contains("refactor") {
                        "♻️"
