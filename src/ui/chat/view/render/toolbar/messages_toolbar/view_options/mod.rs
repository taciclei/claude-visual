...
impl ChatView {
    ...
    pub fn render_view_options(
        ...
        div()
            ...
            .items_center()
            .gap_2()
            .child(render_timestamp_toggle(
                show_timestamps,
                &colors,
                on_timestamp_click,
            ))
            .child(render_compact_toggle(
                compact_mode,
                &colors,
                on_compact_click,
            ))
            .child(render_auto_scroll_toggle(
                auto_scroll,
                &colors,
                on_auto_scroll_click,
            ))
            .child(render_word_wrap_toggle(
                word_wrap,
                &colors,
                on_word_wrap_click,
            ))
            .child(render_line_numbers_toggle(
                show_line_numbers,
                &colors,
                on_line_numbers_click,
            ))
            ...
            .child(render_command_palette_toggle(
                &colors,
                on_command_palette_click,
            ))
            .child(render_shortcuts_toggle(&colors, on_shortcuts_click))
            .when(
                self.context_used > 0 || self.session_input_tokens > 0,
                |d| d.child(self.render_context_usage_indicator(theme, cx)),
            )
    }
}
