...
impl ChatView {
    pub fn render_status_header(&self, theme: &crate::app::theme::Theme, cx: &mut Context<Self>) -> Div {
        div()
            .w_full()
            .flex()
            .items_center()
            ...
            .child(
                div()
                    ...
                    .when_some(self.session_info.as_ref().filter(|i| !i.tools.is_empty()), |d, info| {
                        d.child(self.render_tools_badge(info, theme, cx))
                    })
                    // Skills/commands count (when available) - clickable to show session details
                    .when_some(self.session_info.as_ref().filter(|i| !i.skills.is_empty() || !i.slash_commands.is_empty()), |d, info| {
                        d.child(self.render_skills_badge(info, theme, cx))
                    })
                    // Agents count (when available) - clickable to show session details
                    .when_some(self.session_info.as_ref().filter(|i| !i.agents.is_empty()), |d, info| {
                        d.child(self.render_agents_badge(info, theme, cx))
                    })
                    ...
                    .when_some(self.session_info.as_ref().filter(|i| !i.cwd.is_empty()).cloned(), |d, info| {
                        d.child(self.render_cwd_badge(&info, theme))
                    })
                    // MCP servers status (when available)
                    .when_some(self.session_info.as_ref().filter(|i| !i.mcp_servers.is_empty()), |d, info| {
                        d.child(self.render_mcp_badge(info, theme))
                    })
                    // Session ID badge (when available) - Click to copy full ID
                    .when_some(self.session_info.as_ref().map(|i| i.session_id.clone()).filter(|id| !id.is_empty()), |d, session_id| {
            ...
            .child(
                div()
                    ...
                    .children(self.active_tasks.iter().enumerate().take(3).map(|(idx, task)| {
                        self.render_active_task(idx, task, theme, cx)
                    }))
                    ...
                    .when_some(
                        ...
                        |d, info| {
                            d.child(self.render_session_id_badge_right(info, theme))
                        }
                    )
                    ...
                    .when_some(
                        ...
                        |d, info| {
                            d.child(self.render_version_badge(info, theme))
                        }
                    )
