...
impl AgentWorkspace {
    ...
    pub fn request_approval(
        ...
        self.add_log(LogLevel::Warning, format!("Approval required: {} ({})", description, risk_level));
        cx.emit(AgentWorkspaceEvent::ApprovalRequired(description, risk_level));
    ...
    pub fn approve(&mut self, cx: &mut Context<Self>) {
        if let Some(approval) = self.pending_approval.take() {
            self.add_log(LogLevel::Info, format!("Approved: {}", approval.step_description));
    ...
    pub fn reject(&mut self, reason: impl Into<String>, cx: &mut Context<Self>) {
        ...
        if let Some(approval) = self.pending_approval.take() {
            self.add_log(LogLevel::Warning, format!("Rejected: {} - {}", approval.step_description, reason));
            self.mode = AgentMode::Paused;
            cx.notify();
        }
    }
