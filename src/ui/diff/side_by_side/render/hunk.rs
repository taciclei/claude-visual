...
impl SideBySideDiffView {
    ...
    pub(super) fn render_hunk_side_by_side(&self, hunk: &ManagedHunk, theme: &crate::app::theme::Theme, cx: &Context<Self>) -> impl IntoElement {
        let hunk_id = hunk.id;
        ...
        div()
            ...
            .overflow_hidden()
            .border_1()
            .border_color(if is_focused { accent_color } else { border_color })
            .mb_2()
            // Hunk header
            .child(
                div()
                    ...
                    div()
                        ...
                        .child(
                            div()
                                ...
                                .border_r_1()
                                .border_color(border_color)
                                .children(old_lines.iter().enumerate().map(|(idx, (line_num, content, is_change))| {
                                    let bg = if *is_change {
                                    ...
                                    div()
                                        ...
                                        .child(
                                            div()
                                                .id(SharedString::from(format!("old-line-{}-{}", hunk_id, idx)))
                                                .flex_1()
                                                .px_2()
                                                .py_0p5()
                                                .text_xs()
                                                .font_family("JetBrains Mono")
                                                .whitespace_nowrap()
                                                .text_color(if *is_change { error_color } else { text_color })
                                                .child(if content.is_empty() { " ".to_string() } else { content.clone() }),
                                        )
                                })),
                        )
                        // New side (right)
                        .child(
                            div()
                                .flex_1()
                                .children(new_lines.iter().enumerate().map(|(idx, (line_num, content, is_change))| {
                                    let bg = if *is_change {
                                    ...
                                    div()
                                        ...
                                        .child(
                                            div()
                                                .id(SharedString::from(format!("new-line-{}-{}", hunk_id, idx)))
                                                .flex_1()
                                                .px_2()
                                                .py_0p5()
                                                .text_xs()
                                                .font_family("JetBrains Mono")
                                                .whitespace_nowrap()
                                                .text_color(if *is_change { success_color } else { text_color })
                                                .child(if content.is_empty() { " ".to_string() } else { content.clone() }),
                                        )
                                })),
                        ),
