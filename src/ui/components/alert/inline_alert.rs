...
impl RenderOnce for InlineAlert {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        let (accent, bg) = match self.alert_type {
            AlertType::Info => (hsla(210.0 / 360.0, 0.8, 0.5, 1.0), hsla(210.0 / 360.0, 0.8, 0.5, 0.1)),
            AlertType::Success => (hsla(145.0 / 360.0, 0.6, 0.4, 1.0), hsla(145.0 / 360.0, 0.6, 0.4, 0.1)),
            AlertType::Warning => (hsla(45.0 / 360.0, 0.9, 0.5, 1.0), hsla(45.0 / 360.0, 0.9, 0.5, 0.1)),
            AlertType::Error => (hsla(0.0, 0.7, 0.5, 1.0), hsla(0.0, 0.7, 0.5, 0.1)),
        };

        let icon = self.icon.unwrap_or_else(|| self.alert_type.icon().to_string());
