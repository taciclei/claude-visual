...
impl Render for Table {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .w_full()
            .rounded(px(8.0))
            .when(show_borders, |d| d.border_1().border_color(border_color))
            .overflow_hidden()
            // Header
            .when(self.show_header, |d| {
                ...
                d.child(
                    div()
                        ...
                        .items_center()
                        .bg(surface_hover)
                        .when(show_borders, |d| d.border_b_1().border_color(border_color))
                        .children(self.columns.iter().map(|col| {
                            let sort_icon = match &self.sort {
                                Some(sort) if sort.column == col.key => match sort.direction {
                            ...
                            div()
                                ...
                                .when(is_bordered, |d| d.border_r_1().border_color(border_color))
                                .when(col.sortable, |d| {
                                    d.cursor_pointer().hover(|s| s.bg(surface_color))
                                })
                                .child(
                                    div()
                                        .flex_1()
                                ...
                                .when_some(sort_icon, |d, icon| {
                                    d.child(
                                        div().ml_1().text_xs().text_color(accent_color).child(icon),
                                    )
                                })
                        })),
                )
            ...
            // Body
            .child(div().id("table-body").w_full().flex().flex_col().children(
                self.rows.iter().enumerate().map(|(row_idx, row)| {
                    ...
                    div()
                        ...
                        .when(!row.disabled, |d| d.cursor_pointer().on_click(on_row_click))
                        .children(row.cells.iter().enumerate().map(|(col_idx, cell)| {
                            ...
                            div()
                                ...
                                .when(col.map(|c| c.width.is_none()).unwrap_or(true), |d| {
                                    d.flex_1()
                                })
                                .when(is_bordered, |d| d.border_r_1().border_color(border_color))
                                .child(
