...
pub fn render_content(
    hunk_data: Vec<(
        ...
        String,
        bool,
        Vec<(char, String, Option<usize>, Option<usize>, &'static str)>,
    )>,
    border_color: Hsla,
    surface_color: Hsla,
    text_color: Hsla,
    ...
    div()
        ...
        .children(
            hunk_data
                .into_iter()
                .map(move |(_hunk_idx, _header, _hunk_collapsed, lines)| {
                    div().w_full().children(lines.into_iter().map(
                        move |(prefix, content, old_ln, new_ln, line_type)| {
                            render_line(
                                prefix,
                                content,
                                old_ln,
...
pub fn prepare_hunk_data(
    hunks: &[super::types::DiffHunk],
) -> Vec<(
    usize,
    String,
    bool,
                ...
                .map(|line| {
                    match line {
                        ...
                        DiffLine::Added { content, new_line } => {
                            ('+', content.clone(), None, Some(*new_line), "added")
                        }
                        DiffLine::Removed { content, old_line } => {
                            ('-', content.clone(), Some(*old_line), None, "removed")
                        }
                        DiffLine::HunkHeader { header, .. } => {
                            ('@', header.clone(), None, None, "header")
                        }
                    }
                })
                .collect();
